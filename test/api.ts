...
import YOUTransaction from "../src/youTransaction";
...
tape('[Transaction]: Basic functions', function(t) {
  ...
  t.test('should sign tx', function(st) {
    ...
    t.test('should sign tx with YOUTransaction', function(st) {
        ...
        tx2.sign(privateKey);
        const serializedTx2 = tx2.serialize().toString();
        st.comment("YOUTransaction serializedTx " + (serializedTx === serializedTx2));
        st.comment("YOUTransaction getSenderAddress " + (tx.getSenderAddress().toString('hex') === tx2.getSenderAddress().toString('hex')));
        st.comment("YOUTransaction getSenderPublicKey " + (tx.getSenderPublicKey().toString('hex') === tx2.getSenderPublicKey().toString('hex')));

        st.end()
    });
  t.test('should sign tx with YOUTransaction', function(st) {
      youTransactions.forEach(function(tx, i) {
          if (txFixtures[i].privateKey) {
              const privKey = new Buffer(txFixtures[i].privateKey, 'hex')
              tx.sign(privKey)
          }
  ...
  t.test('should sign tx with YOUTransaction', function(st) {
      ...
      tx.sign(privateKey);
      st.comment("chainId "+ tx.getChainId())
      st.equals(tx.getChainId(), 123)
